...
import { formatDate, ITEMS_PER_PAGE, truncate } from "@/lib/utils"
    ...
    return (
        <>
            <div className="bg-gradient-to-r from-primary-800 to-primary-600 py-20">
                <FadeIn direction="up" className="container-custom text-center">
                        ...
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {blogs.map((blog, index) => (
                                    <FadeIn key={blog.id} delay={index * 0.1} direction="up" className="flex">
                                        <Link href={`/blog/${blog.slug}`} className="w-full group card hover:shadow-xl transition-all duration-300 flex flex-col">
                                            {blog.image && (
                            ...
                            {totalPages > 1 && (
                                <FadeIn direction="up" delay={0.2} className="flex justify-center items-center gap-3 mt-12">
